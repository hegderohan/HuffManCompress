...
public class ImplementationClassForCompression implements Compress {
    ...
    @Override
    public void getByteArray(Map<Character, String> huffmanMap) {

        StringBuilder codedString = new StringBuilder();
        FileReader f1;
        try {

            f1 = new FileReader(Path.inputFilePath);
        } catch (FileNotFoundException e) {
            throw new RuntimeException(e);
        }
        int val = 0;
        ...
        while (val != -1) {
            codedString.append(HuffMan_Map.get((char) val));
            try {
                val = f1.read();
            } catch (IOException e) {
                throw new RuntimeException(e);
        ...
        int no_of_zeros = 8 - (codedString.length() % 8);
        while (no_of_zeros != 0) {
            codedString.append('0');
            no_of_zeros--;
        }
        byte[] bytearray = new byte[codedString.length() / 8];
        String sub = "";
        int bytearray_indx = 0;
        for (int i = 0; i < codedString.length(); i = i + 8) {
            int j = 0;
            while (j < 8) {
                sub = sub + (codedString.charAt(i + j));
                j++;
            }
